...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        if (options.getProfilePicUrl) {
            const pfpUrl = await options.getProfilePicUrl(message.groupInvite.jid, 'image');
            if (pfpUrl) {
    ...
    if ('sections' in message && !!message.sections) {
        const listMessage = {
            ...
            listType: message.hasOwnProperty('listType') ? message.listType : WAProto_1.proto.Message.ListMessage.ListType.SINGLE_SELECT
        };
        m = {
            listMessage
        };
    ...
    if ('interactiveButtons' in message && !!message.interactiveButtons) {
        ...
        const document = message?.header?.document ?
            await (0, exports.prepareWAMessageMedia)({
                    document: message?.header?.document,
                    ...options
                },
                options
            ) :
    ...
    if ("cards" in message && !!message.cards) {
        const cards = await Promise.all(
            message.cards.map(async slide => {
                ...
                if (type === "image") {
                    media = await (0, exports.prepareWAMessageMedia)({
                        image: {
                            url
                        },
                        ...options
